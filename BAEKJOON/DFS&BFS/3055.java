...
public class Main {
    ...
    static void bfs(int x, int y) {
        Queue<int[]> queue1 = new LinkedList<int[]>(); //고슴도치 좌표
        Queue<int[]> queue2 = new LinkedList<int[]>(); //물 좌표
        for(int j = 0; j<n; j++) {
            for(int k = 0; k<m; k++) {
                if(map[j][k].equals("S") && !visit1[j][k]) {
        ...
        while(!queue1.isEmpty()) {
            ...
            for(int a = 0; a<size; a++) {
                ...
                for(int i = 0; i<4; i++) {
                    ...
                    if(map[nx][ny].equals("D") || map[nx][ny].equals("X")) //소굴이면 건너뜀
                        continue;
            ...
            for(int a = 0; a<size1; a++) {
                ...
                for(int i = 0; i<4; i++) {
                    ...
                    if(map[nx][ny].equals("*") || map[nx][ny].equals("X")) //소굴이면 건너뜀
                        continue;
