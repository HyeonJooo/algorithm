...
public class Main {
    ...
    static void bfs(int x, int y) {
        Queue<int[]> queue1 = new LinkedList<int[]>(); //지훈 좌표
        Queue<int[]> queue2 = new LinkedList<int[]>(); //불 좌표
        for(int j = 0; j<n; j++) {
            for(int k = 0; k<m; k++) {
                if(map[j][k].equals("J") && !visit1[j][k]) {
                    queue1.offer(new int[] {j,k});
                    visit1[j][k] = true;
                }
                if(map[j][k].equals("F") && !visit2[j][k]) {
        ...
        while(!queue1.isEmpty()) {
            ...
            for(int a = 0; a<size1; a++) {//지훈
                int qx = queue1.peek()[0];
                int qy = queue1.peek()[1];
                queue1.poll();
                if(map[qx][qy].equals("F"))
                    continue;
                ...
                for(int i = 0; i<4; i++) {
                    ...
                    if(map[nx][ny].equals("J") || map[nx][ny].equals("#"))
                        continue;
